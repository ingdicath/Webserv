...
void    RequestTester::DELETERequest(int &sock) {
    std::string rqs =
            "DELETE /images/testPOSTRequest.txt HTTP/1.1\r\n"\
        "Host: localhost\r\n"\
        "\r\n";
    send(sock, rqs.c_str(), strlen(rqs.c_str()), 0);
    ...
    std::cout << "DELETE Request test done" << std::endl;
    return;
}
...
int main(int argc, char **argv) {
    if (argc != 2) {
        ...
        std::cout << "0:  basic GET request, without body" << std::endl;
        std::cout << "1:  basic POST request, with content_length and body" << std::endl;
        std::cout << "2:  Long GET request" << std::endl;
        std::cout << "3:  Long POST request" << std::endl;
        std::cout << "4:  DELETE request" << std::endl;
